    ...
    list_team_expired: async (Student_id, year_semester) => {
        try {
            const teams = await new Promise((resolve, reject) => {
                ...
                ON A.Student_id = E.Student_id and A.Course_id = D.Course_id and A.Student_id = ? and A.year_semester = ? and A.Course_id = B.Course_id and B.Team_id = C.Team_id and C.Student_id = ? and C.voted = 0;`, [Student_id, year_semester, Student_id], (error, rows) => {
                    if (error) {
                        console.error(error);
                        reject(error);
                    } else {
